...
def _normalize_cycle_ratios(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - 1e-9 ≤ abs(sum - 1.0) < 0.0005 : float precision — auto-normalize + warn
    - abs(sum - 1.0) ≥ 0.0005        : data error — all offending groups are
    ...
    ------
    ValueError
        If one or more (차수, Sender CC) groups deviate from 1.0 by ≥ 0.0005.
