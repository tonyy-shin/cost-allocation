...
def test_result_row_count(pipeline_outputs, loaded_inputs):
    n_pairs = (
        loaded_inputs["raw_coa_df"]
        .drop_duplicates(["COA", "Cost Center"])
        .shape[0]
    )
    assert len(pipeline_outputs["result"]) == n_pairs
...
def test_only_expected_residual_warnings_on_happy_path(loaded_inputs):
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        build_result(decomposed, df_direct, raw_coa_df,
                     cycle_df["차수"].nunique())
